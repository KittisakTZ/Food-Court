...
const STATUS_CFG: Record<Order['status'], { label: string; bar: string; badge: string; text: string }> = {
    PENDING:               { label: 'Pending',                     bar: 'bg-amber-400',   badge: 'bg-amber-100 text-amber-800',   text: 'text-amber-700' },
    AWAITING_PAYMENT:      { label: 'Awaiting Payment',            bar: 'bg-blue-400',    badge: 'bg-blue-100 text-blue-800',     text: 'text-blue-700' },
    AWAITING_CONFIRMATION: { label: 'Awaiting Slip Confirmation',   bar: 'bg-purple-400',  badge: 'bg-purple-100 text-purple-800', text: 'text-purple-700' },
    COOKING:               { label: 'Cooking',                     bar: 'bg-orange-500',  badge: 'bg-orange-100 text-orange-800', text: 'text-orange-700' },
    READY_FOR_PICKUP:      { label: 'Ready for Pickup!',           bar: 'bg-emerald-500', badge: 'bg-emerald-100 text-emerald-800', text: 'text-emerald-700' },
    COMPLETED:             { label: 'Completed',                   bar: 'bg-slate-400',   badge: 'bg-slate-100 text-slate-600',   text: 'text-slate-600' },
    CANCELLED:             { label: 'Cancelled',                   bar: 'bg-red-400',     badge: 'bg-red-100 text-red-800',       text: 'text-red-700' },
    REJECTED:              { label: 'Rejected',                    bar: 'bg-red-400',     badge: 'bg-red-100 text-red-800',       text: 'text-red-700' },
};
...
const SellerCountdown = ({ estimatedReadyAt }: { estimatedReadyAt: string }) => {
    ...
    return (
        <span className={`font-mono font-bold text-sm tabular-nums ${done ? 'text-emerald-600' : soon ? 'text-amber-500 animate-pulse' : 'text-orange-600'}`}>
            {done ? 'Soon' : `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`}
        </span>
    );
};
...
const CancelOrderModal = ({ order, onClose }: { order: Order; onClose: () => void }) => {
    ...
    const presetReasons = ['Insufficient ingredients', 'Out of stock', 'Store closed', 'Menu temporarily suspended', 'Unable to accept order'];
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                <div className="bg-gradient-to-r from-red-500 to-rose-500 p-5 rounded-t-2xl text-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold">Cancel Order #{order.queueNumber}</h3>
                            <p className="text-red-100 text-sm">Customer: {order.buyer?.username} · Customer will be notified</p>
                        </div>
                ...
                <div className="p-5 space-y-4">
                    <p className="text-sm text-slate-600 font-medium">Select a reason for cancellation:</p>
                    <div className="space-y-2">
                        ...
                        <label className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedReason === '__custom__' ? 'border-red-400 bg-red-50' : 'border-slate-200 hover:border-slate-300'}`}>
                            ...
                            <span className="text-sm font-medium text-slate-800">Other (Specify)</span>
                        </label>
                        {selectedReason === '__custom__' && (
                            <input type="text" value={customReason} onChange={e => setCustomReason(e.target.value)} placeholder="Specify reason..." className="w-full px-4 py-2.5 border-2 border-slate-300 rounded-xl text-sm focus:outline-none focus:border-red-400" autoFocus />
                        )}
                    </div>
                    <div className="flex gap-3 pt-2">
                        <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border-2 border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition-all">Close</button>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 p-5 rounded-t-2xl text-white flex-shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-orange-100 text-sm">Queue #{order.queueNumber} · {order.buyer?.username}</p>
                    </div>
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-4">
                ...
                <div>
                    <h4 className="font-bold text-slate-700 mb-3 text-sm">Order Items ({order.orderItems.length} items)</h4>
                    <div className="space-y-2">
                ...
                {order.description && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                        <p className="font-semibold text-amber-800 text-sm mb-1">Note from customer</p>
                ...
                <div className="bg-orange-50 rounded-xl p-4 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-slate-400">{order.paymentMethod === 'PROMPTPAY' ? 'PromptPay' : 'Cash'}</p>
                    </div>
                    <p className="text-2xl font-black text-orange-600">฿{order.totalAmount.toFixed(0)}</p>
                </div>
                {order.paymentSlip && (
                    <div>
                        <p className="font-semibold text-slate-700 text-sm mb-2">Payment Slip</p>
            ...
            <div className="p-5 border-t border-slate-100 flex-shrink-0">
                <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-slate-800 text-white font-bold text-sm hover:bg-slate-700 transition-all">Close</button>
            </div>
        </div>
    </div>
);
        ...
        <div ref={setNodeRef} style={style}>
            ...
            <div className={`bg-white rounded-2xl shadow-sm border transition-all overflow-hidden
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 mb-1">
                                ...
                                <span className="font-bold text-slate-800 truncate">{order.buyer?.username ?? 'Customer'}</span>
                            </div>
                        ...
                        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                            <button onClick={() => setShowDetailModal(true)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all" title="View Details">
                    ...
                    {order.scheduledPickup && (
                        <div className="mt-2.5 flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
                            ...
                            <span className="text-xs font-bold text-amber-700">
                                Pickup Time: {new Date(order.scheduledPickup).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false })}
                            </span>
                        </div>
                    )}
                </div>
                ...
                {order.hasIssue && order.issueReason && (
                    <div className="mx-4 mb-3 flex items-center gap-2.5 p-3 bg-red-50 border border-red-200 rounded-xl">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs font-bold text-red-700 uppercase tracking-wide">Order Issue</p>
                    ...
                    return (
                        <div className="mx-4 mb-3 rounded-xl overflow-hidden border border-purple-200 bg-purple-50">
                            <div className="px-4 py-2.5 bg-purple-100 border-b border-purple-200">
                                <p className="font-bold text-purple-800 text-sm">Payment Slip — Please Verify</p>
                            </div>
                            <div className="p-3 relative group cursor-pointer" onClick={() => window.open(slipUrl, '_blank')}>
                                ...
                                <div className="hidden p-6 bg-red-50 rounded-lg border border-red-200 items-center justify-center text-red-500 text-sm font-medium">Failed to load slip</div>
                ...
                <div className="mx-4 mb-4 flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                    <div>
                        <p className="text-xs text-slate-400">{totalItems} items · Prep time ~{totalCookingMin} mins</p>
                ...
                <div className="px-4 pb-3 border-t border-slate-100 pt-3">
                    <div className="flex items-center justify-center gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                            <span className="text-xs text-slate-400 font-medium">Go to Q</span>
                            <input
                                type="number"
                                value={jumpPosition}
